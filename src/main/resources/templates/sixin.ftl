...
<head>
    <script type="text/javascript">
        ...
        if(isIE()){
            ...
            //    script.src ='https://g.csdnimg.cn/browser_upgrade/1.0.2/browser_upgrade.js'
        }
    </script>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta name="renderer" content="webkit">
    <title>私信-消息-CSDN</title>
    ...
    <script type="text/javascript" src="https://g.csdnimg.cn/user-login/2.1.6/user-login.js"></script>
    <!--&lt;!&ndash;[if IE]>-->
...
<body>
<script id="toolbar-tpl-scriptId" prod="download" skin="black" src="https://csdnimg.cn/public/common/toolbar/js/content_toolbar.js" type="text/javascript" domain="http://blog.csdn.net"></script>
<div class="toast_div" style="position:fixed;top:50px;margin:0 auto;z-index:999"></div>
...
<div class="wrapper box-sizing">
    <div class="content">
        <div class="left-panel" id="left-panel">
            ...
            <div class="title clearfix" onclick="hideCancel()" >
                <img src="" alt="" class="radius-circle avatar" id="userPic" />
                <span id="userName">&nbsp;</span>
                <img src="images/setting.png" class="setting" id="setting"/>
                <div class="pull_box" id="pull_box"></div>
                <!-- <span>
                    <img src="images/pen.png" class="u-modify" id="showMyInfo">
                </span> -->
                <!--<span class="exit" title="退出" id="logout">退出</span>-->
            </div>
            <div class="left-panel-title-border"></div>
            <div class="m-devices hide">正在使用 CSDN web版</div>
            <!-- 隐藏 聊天 好友 群 切换 tab -->
            ...
            <div class="item" id="sessionsWrap" data-type="sessions">
                ...
                <div class="list">
                    ...
                    <div id="sessions">
                        <p class="session-empty">正在获取最近联系人...</p>
                    </div>
                </div>
            </div>
        ...
        <div class="right-panel" id="right_blank">
        ...
        <div class="right-panel hide" id="rightPanel">
            ...
            <div class="chat-box show-netcall-box" id="chatBox">
                ...
                <div class="chat-editor box-sizing" id="chatEditor" data-disabled="1">
                    ...
                    <textarea id="messageText" class="chat-btn msg-input box-sizing p2p" rows="1" autofocus="autofocus" maxlength="500"></textarea>
                    ...
                    <div class="chat-editor-bottom f-cb">
                        <span class="chat-editor-tips f-fr">按下Enter发送内容</span>
                    </div>
    ...
    <div class="m-blacklist radius5px hide" id="blacklist">
        ...
        <ul class="f-cb list">
            <li class="items f-cb">
                <img src="images/default_header.png" class="head">
...
<script src="../3rd/NIM_Web_SDK_v6.8.0.js"></script>
...
<!--<script src="../3rd/jquery-1.11.3.min.js"></script>-->
<script src="../3rd/jquery.picEyes.js"></script>
<script src="../3rd/toast.js"></script>
<script src="../3rd/md5.js"></script>
...
<script src="js/module/message.js?1234567"></script>
...
<script id="whiteboard-template" type="text/x-template">
    <div v-show="display" class="function-box whiteboard">
        ...
        <div v-if="!connected && isCalling">
            <img :src="avatar || 'images/default_header.png'" alt="用户头像" class="avatar">
...
<script>
    ...
    $('body').on('click',function(e){
        if(e.target.id&&e.target.id==='setting'){
    ...
    $('.setting').on('click',function(){
        if(!CSDN_flag) {show='none'} else {
            show='block'
        }
        $('.pull_box').css('display',show)
        CSDN_flag = !CSDN_flag
    })
    $('.pull_box').on('click',function(){
        var html = $('.pull_box').html()
        var flag = $('.pull_box').html()&&$('.pull_box').html().indexOf('允许')>-1?false:true
        if(flag){
            setAttention()
        } else {
            cancelSetAttention()
    ...
    function setAttention(){
        $.ajax({
            url: CONFIG.API_MSG_URL + 'v1/im/onlyAllowAttention/setAttention',
            type: 'POST',
            ...
            success: function(data) {
                if (data.status) {
                    console.log('设置只允许我关注的人私信我成功')
                    $('.toast_div').toast({
                        content: data.message,
                        duration:2000
                    });
                    $('.pull_box').html('允许陌生人私信我')
                } else {
                    console.log('设置只允许我关注的人私信我失败')
                    $('.toast_div').toast({
                        content: data.message,
                        duration:2000
                    });
    ...
    function cancelSetAttention(){
        $.ajax({
            url: CONFIG.API_MSG_URL + 'v1/im/onlyAllowAttention/cancelSetAttention',
            type: 'POST',
            // data:JSON.stringify(param),
            contentType: 'application/json',
            dataType: 'json',
            ...
            success: function(data) {
                if (data.status) {
                    console.log('取消设置只允许我关注的人私信我成功')
                    $('.toast_div').toast({
                        content: data.message,
                        duration:2000
                    });
                    $('.pull_box').html('拒绝陌生人私信我')
                } else {
                    console.log('取消设置只允许我关注的人私信我失败')
                    $('.toast_div').toast({
                        content: data.message,
                        duration:2000
                    });
    ...
    function getAttention (){
        $.ajax({
            url: CONFIG.API_MSG_URL + 'v1/im/onlyAllowAttention/getAttention',
            type: 'POST',
            // data:JSON.stringify(param),
            contentType: 'application/json',
            dataType: 'json',
            ...
            success: function(data) {
                // console.log('返回的getAttention数据data=====',data)
                if (data.status) {
                    if(data.data.isSetAttention){
                        // 当前是允许陌生人私信我   提示拒绝陌生人私信我   发拒绝的请求
                        $('.pull_box').html('允许陌生人私信我')
                    } else {
                        // 发允许的请求
                        $('.pull_box').html('拒绝陌生人私信我')
